...
async def dl_handler(client: Client, message: Message):
    ...
    # 1. Parse Flags (-e for extract, -z for zip)
    ...
    if match:
        ...
        return await message.reply_text(
            "❌ **Usage:**\n"
            "• `/dl https://link.com`\n"
            "• `/dl -z link` (Zip before upload)\n"
            "• `/dl -e link` (Extract archive)\n"
            "• Or Reply to a link with `/dl`"
        )
    ...
    if "https://t.me/" in link_text:
        ...
        # Send status msg first to avoid 'NoneType' errors later
        status = "🤐 **Zipping & Uploading**" if should_zip else "⬇️ **Queued External Download**"
        if should_extract: status = "📂 **Extracting & Uploading**"
...
# --- THE FIXED DOWNLOADER ENGINE ---
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    # FIX: We use 'status_message' everywhere now to avoid NameError
    ...
    try:
        ...
        if ttype == 'torrent':
            ...
            # Select specific file index if requested
            if sel != 'all': 
                aria2.client.change_option(gid, {"select-file": sel})
            ...
            # FIX: Use client.remove(gid) here too
            try: aria2.client.remove(gid)
            except: pass
            ...
            if m_login:
                ...
                raise Exception("Mega Login Failed (IP Blocked?)")

        else:
            if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
            
            # FIX: Gofile and Mediafire added here
            video_sites = [
                "youtube.com", "youtu.be", 
                "instagram.com", "tiktok.com", "twitch.tv", 
                "facebook.com", "vimeo.com", "dailymotion.com", 
                "gofile.io", "mediafire.com"
            ]
            ...
            if is_video_site:
                ...
                await status_message.edit("⬇️ **Direct Link Detected...**\nRunning Pure Aria2...")
                cmd = [
                    "aria2c", link,
                    "-d", str(down_path),
                    "-x", "16", "-s", "16", "-k", "1M",
        ...
        # 2. ZIPPING (-z) - Uses Python's zipfile
        if should_zip:
            await status_message.edit("📦 **Zipping Files...**")
