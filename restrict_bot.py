...
async def dl_handler(client: Client, message: Message):
    ...
    # Check for flags (e.g., /dl -e link or /dl link -e)
    args = message.text.split()
    if "-e" in args:
        should_extract = True
        args.remove("-e")
    
    # Logic to get the link
    if message.reply_to_message and (message.reply_to_message.text or message.reply_to_message.caption):
        link_text = message.reply_to_message.text or message.reply_to_message.caption
    elif len(args) > 1:
        link_text = args[1] # The second word is the link
        
    # Clean link (remove surrounding text if mixed)
    # Simple regex to find http link
    ...
    if "https://t.me/" in link_text:
        # Telegram Link -> Use existing logic wrapper
        # For simplicity, we just trigger the 'save' handler logic or direct queue
        PENDING_TASKS[user_id] = {"link": link_text, "status": "waiting_choice"}
        # ... (Show buttons same as save handler) ...
        buttons = [
        ...
        # External Link -> Add extraction flag
        # We bypass the "Menu" and go straight to queue for /dl command
        task_data = {
            "link": link_text,
            "type": "external",
            "quality": "best", # Default for /dl
            "zip": False, # We don't zip BY DEFAULT
            "extract": should_extract # NEW FLAG
        }
        
        await message.reply(f"⬇️ **Queued External Download**\nExtract: `{should_extract}`\nLink: `{link_text[:30]}...`")
        asyncio.create_task(start_task_final(client, message, task_data, delay=3, user_id=user_id))
