    ...
    try:
        ...
        if ttype == 'torrent':
            ...
            try:
                ...
                # Silent fail for mega anonymous login to prevent crash
                pass
            ...
            # FIX: Added gofile.io and mediafire to force yt-dlp processing
            video_sites = [
                "youtube.com", "youtu.be", "instagram.com", "tiktok.com", "twitch.tv", 
                "facebook.com", "vimeo.com", "dailymotion.com", "gofile.io", "mediafire.com"
            ]
            is_video_site = any(site in link.lower() for site in video_sites)
            cookie_path = await db.get_cookies()

            if is_video_site:
                await status_msg.edit("🔎 **Processing Link...**\n(Using yt-dlp)")
        ...
        # 2. ZIPPING (-z) - FIXED: Uses python zipfile (No OS dependency)
        if should_zip:
        ...
        # ======================================================================
        # ☁️ UPLOAD PHASE (With FloodWait & Sleep Protection)
        # ======================================================================
        ...
        for i, file_path in enumerate(files, start=1):
            ...
            # Start status updater for this file (Updates every 20s to avoid spam)
                ...
                try:
                    if fsize <= SPLIT_SIZE:
                        ...
                        while start_byte < fsize:
                            ...
                            # Stream upload (No extra disk space used)
                    ...
                    # FIX: Sleep exactly as long as Telegram says, then retry
                    wait_time = e.value + 5
                    print(f"⚠️ FloodWait: Sleeping {wait_time}s...")
                    await status_msg.edit(f"⏳ **FloodWait:** Sleeping {wait_time}s...")
                    await asyncio.sleep(wait_time)
                
                except Exception as e:
                    print(f"❌ Upload Error: {e}")
                    # If it's a network error, maybe retry? 
                    # For safety, we break here so we don't loop forever on a broken file.
                    break
            
            # Delete file immediately after upload to free space
