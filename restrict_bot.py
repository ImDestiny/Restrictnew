...
async def dl_handler(client: Client, message: Message):
    ...
    if match:
        ...
        # (Only if it was in the main message)
    ...
    # 2. PARSE FLAGS (-z, -e)
    # We remove them from text after finding them to clean up the string
    should_zip = False
    ...
    # This regex grabs everything after "-n" until the end of the line
    custom_name = None
...
async def yt_quality_cb(client, query):
    ...
    # Update data with selected quality
    task_data = {
        "link": data['link'],
        "type": "youtube", # Special type for our new logic
        "quality": quality,
        "zip": False # YouTube videos usually don't need zipping
    }
...
async def handle_general_link(client, message, link):
    ...
    # 1. ROBUST YOUTUBE DETECTION
    # Checks for standard youtube, youtu.be, AND your googleusercontent proxy links
    is_youtube = False
    if "youtube.com" in link or "youtu.be" in link:
        is_youtube = True
    elif "googleusercontent.com" in link and "youtube" in link:
        is_youtube = True
        
    if is_youtube:
        EXTERNAL_QUEUE[user_id] = {"link": link, "type": "youtube"}
        
        # Grid of Resolutions
        buttons = [
            [InlineKeyboardButton("🌟 4K (2160p)", callback_data="yt_q:2160"), InlineKeyboardButton("✨ 2K (1440p)", callback_data="yt_q:1440")],
            [InlineKeyboardButton("📺 1080p", callback_data="yt_q:1080"), InlineKeyboardButton("🖥️ 720p", callback_data="yt_q:720")],
            [InlineKeyboardButton("📼 480p", callback_data="yt_q:480"), InlineKeyboardButton("📱 360p", callback_data="yt_q:360")],
            [InlineKeyboardButton("🎵 Audio Only", callback_data="yt_q:audio")]
        ]
        
        await msg.edit(
            "🎬 **YouTube Link Detected!**\n\n"
            "Select the **Maximum Quality** you want.\n"
            "__This applies to single videos AND playlists.__", 
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        return

    # 2. STANDARD LOGIC (For other sites like Twitch, FB, etc.)
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            if 'entries' in info:
                ...
                await msg.edit("🔗 **File Link.**", reply_markup=InlineKeyboardMarkup(buttons))

    except Exception as e:
        # Fallback if yt-dlp fails to analyze
        EXTERNAL_QUEUE[user_id] = {"link": link, "type": "file"}
        buttons = [[InlineKeyboardButton("📁 Upload", callback_data="ext_zip:no")]]
