...
async def dl_handler(client: Client, message: Message):
    ...
    # --- NEW: YOUTUBE DETECTION (Inserted Here) ---
    # This checks for standard YT, Shortened YT, and your Proxy links
    is_youtube = any(x in link_text for x in ["youtube.com", "youtu.be", "googleusercontent.com/youtube", "youtube.com/watch", "youtu.be/"])
    ...
    if "https://t.me/" in link_text:
        ...
        # Smart Logic: If zipping, ensure name ends in .zip
...
async def handle_general_link(client, message, link):
    ...
    # Check if user cancelled previous setup
    ...
    try:
        ...
        # B. SINGLE VIDEO LOGIC
            ...
            buttons.append([InlineKeyboardButton("🎵 Audio (MP3)", callback_data="yt_q:audio")])
...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    # We define this inside so we can call it repeatedly for playlists
    async def upload_files_in_folder(current_folder, file_list=None):
        ...
        for i, file_path in enumerate(file_list, start=1):
            ...
            if ext in [".mp4", ".mkv", ".webm", ".avi"]:
                ...
                method = client.send_audio
            elif ext in [".jpg", ".png", ".jpeg"]:
                method = client.send_photo
            else:
                method = client.send_document

            # Upload Loop (Retries included)
            upload_attempts = 0
            while True:
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                try:
                    thumb_path = list(current_folder.glob("*.jpg")) # Check for thumbnails
    ...
    try:
        ...
        if ttype == "youtube":
            ...
            ydl_opts = {
                ...
                'writethumbnail': True, 
                'addmetadata': True,
                'embedsubtitles': True, # Embed subs into MKV/MP4
                'outtmpl': str(down_path / '%(title)s.%(ext)s'),
                'restrictfilenames': True, # Cleaner filenames
            }

            # 2. Set Quality String
            if quality == "audio":
                ydl_opts['format'] = "bestaudio/best"
                ydl_opts['postprocessors'] = [{'key': 'FFmpegExtractAudio','preferredcodec': 'mp3','preferredquality': '192'}]
            elif quality.isdigit():
                # Logic: Video <= Quality + Best Audio
                ydl_opts['format'] = f"bestvideo[height<={quality}]+bestaudio/best[height<={quality}]/best"
                ydl_opts['merge_output_format'] = 'mkv' # MKV handles embedded subs best
            else:
                ydl_opts['format'] = "bestvideo+bestaudio/best"
                ydl_opts['merge_output_format'] = 'mkv'

            # 3. Analyze Link (Is it a playlist or video?)
            ...
            for idx, entry in enumerate(entries, 1):
                ...
                # A. Download ONE video
                await status_message.edit(f"⬇️ **Downloading ({idx}/{total_items})**\n`{title}`")
                
                # We create a specific folder for this video to avoid mixing files
                vid_folder = down_path / f"vid_{idx}"
                vid_folder.mkdir(exist_ok=True)
                
                # Update outtmpl for this specific folder
                ...
                # C. Delete ONE video (Folder removal)
        ...
        # ======================================================================
        # ☁️ MEGA / TORRENT / OTHER LOGIC (Legacy Support)
        # ======================================================================
        else:
            # (Run the old download logic for non-YT files here)
            # For brevity, reusing the simple download structure
            
            await status_message.edit("⬇️ **Downloading...**")
            
            # ... [Insert your existing Aria2/Mega code here if needed] ...
            # For this example, assuming generic handled similarly or passed to simple helper
            
            if ttype == 'mega':
                if not m_login: m_login = mega.login()
                m_login.download_url(link, str(down_path))
            
            # After generic download, upload everything in down_path
