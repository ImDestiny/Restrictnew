...
async def process_links_logic(client: Client, message: Message, text: str, dest_chat_id=None, dest_thread_id=None, delay=3, acc_user_id=None, task_uuid=None):
    ...
    # 2. Join Logic (Simplified for brevity, assuming existing logic is fine)
    if "https://t.me/+" in text or "https://t.me/joinchat/" in text:
        # ... (Keep your existing Join Logic here if needed, or rely on previous code) ...
        # For this fix, we focus on the batch processing part below.
        pass 
    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            if filter_thread_id and fromID < filter_thread_id: fromID = filter_thread_id
            ...
            status_message = await client.send_message(
                message.chat.id, f"{status_text_header}\n**Initializing...**", reply_to_message_id=message.id
            )
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                cycle_start = time.time() # <--- Timer Starts for this message
                ...
                try:
                    ...
                    # PROCESS MESSAGE
                    # Note: handle_private does NOT sleep. It raises FloodWait if needed.
                ...
                # STATUS UPDATE (Every 20 msgs or 60s)
                current_time = time.time()
                if (index % 20 == 0) or ((current_time - last_update_time) > 60):
                    last_update_time = current_time
                    try:
                        elapsed_total = current_time - start_time
                        speed = index / elapsed_total if elapsed_total > 0 else 0
                        eta = (total_count - index) / speed if speed > 0 else 0
                ...
                if is_success:
                    sleep_needed = max(0, delay - elapsed_work)
                    await asyncio.sleep(sleep_needed)
                else:
                    await asyncio.sleep(2) # Small penalty for errors
            ...
            if 'was_cancelled' in locals() and was_cancelled: final_msg = "**Task Cancelled** 🛑"
            
            try: await client.send_message(message.chat.id, final_msg, reply_to_message_id=message.id)
            except: pass
