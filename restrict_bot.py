...
try:
    # 1. Clean the inputs (remove spaces) and check if they exist
    email = MEGA_EMAIL.strip() if MEGA_EMAIL else None
    password = MEGA_PASSWORD.strip() if MEGA_PASSWORD else None
    
    if email and password:
        print(f"🔐 Logging into Mega Account: {email[:3]}***")
        m_login = mega.login(email, password)
    else:
        print("👤 No Credentials Found. Logging into Mega Anonymously...")
        m_login = mega.login()
        
except Exception as e:
    print(f"⚠️ Mega Login Error (Using Anonymous): {e}")
    try:
        m_login = mega.login()
    except:
        print("❌ Mega completely failed to initialize.")
...
async def dl_handler(client: Client, message: Message):
    ...
    # Get the full text (command + args)
    full_text = message.text or message.caption or ""
    
    # 1. EXTRACT LINK First (Regex)
    link_text = None
    match = re.search(r'(https?://[^\s]+)', full_text)
    
    # If not in command, check reply
    if not match and message.reply_to_message:
        reply_text = message.reply_to_message.text or message.reply_to_message.caption or ""
        match = re.search(r'(https?://[^\s]+)', reply_text)
        
    if match:
        link_text = match.group(1)
        # Remove link from text so it doesn't mess up name parsing
        if link_text in full_text:
            full_text = full_text.replace(link_text, "")
    else:
        return await message.reply("❌ **No Link Found.**\nUsage: `/dl https://link.com -n Filename.mkv`")

    # --- NEW: YOUTUBE DETECTION ---
    # Matches standard YT, Shortened YT, and Proxy links
    is_youtube = any(x in link_text for x in ["youtube.com", "youtu.be", "googleusercontent.com/youtube"])
    
    if is_youtube:
        # If it's YouTube, we SKIP the rest and go straight to the Quality Menu
        await handle_general_link(client, message, link_text)
        return
    # ------------------------------
    ...
    if re.search(r'\s-z\b', full_text, re.IGNORECASE):
        should_zip = True
        full_text = re.sub(r'\s-z\b', "", full_text, flags=re.IGNORECASE)
        
    if re.search(r'\s-e\b', full_text, re.IGNORECASE):
        should_extract = True
        full_text = re.sub(r'\s-e\b', "", full_text, flags=re.IGNORECASE)

    # 3. EXTRACT CUSTOM NAME (-n / -N)
    custom_name = None
    name_match = re.search(r'\s-n\s+(.+)', full_text, re.IGNORECASE)
    if name_match:
        custom_name = name_match.group(1).strip()

    # --- QUEUE TASK ---
    if "https://t.me/" in link_text:
        PENDING_TASKS[user_id] = {"link": link_text, "status": "waiting_choice"}
        buttons = [
            [InlineKeyboardButton("📂 Send to DM", callback_data="dest_dm")],
            [InlineKeyboardButton("📢 Send to Channel", callback_data="dest_custom")]
        ]
        await message.reply(f"🔗 **Telegram Link:**\n`{link_text}`\n\nWhere to send?", reply_markup=InlineKeyboardMarkup(buttons))
    else:
        if should_zip and custom_name and not custom_name.lower().endswith(".zip"):
            custom_name += ".zip"
        ...
        status = "⬇️ **Queued Download**"
        if should_zip: 
            name_display = custom_name if custom_name else "Archive.zip"
            status = f"🤐 **Zipping** as `{name_display}`"
        elif custom_name: 
            status = f"⬇️ **Downloading** as `{custom_name}`"

        status_msg = await message.reply(status)
        asyncio.create_task(start_task_final(client, status_msg, task_data, delay=3, user_id=user_id))
...
async def start_task_final(client: Client, message_context: Message, task_data: dict, delay: int, user_id: int):
    ...
    start_text = f"✅ **Task Started!**\nDestination: `{dest}`\nSpeed: `{delay}s` delay\nTask ID: `{task_uuid[:8]}`"
    
    # --- CRITICAL FIX: Capture the Status Message ---
    status_message = None
    try:
        if isinstance(message_context, Message):
            # FIX: Check if from_user exists first
            is_bot_msg = message_context.from_user and message_context.from_user.is_bot
            
            if is_bot_msg:
                # It's our own message, edit it
                await message_context.edit(start_text)
                status_message = message_context
            else:
                # It's a user message, reply to it
                status_message = await message_context.reply(start_text, quote=True)
    except Exception as e:
    ...
    # --- LOGIC SWITCHER ---
    # We pass 'status_message' (the one the bot can edit) instead of 'message_context'
    if task_data.get("type") in ["external", "mega", "mega_file", "torrent", "playlist_all"]:
        ...
        asyncio.create_task(
            process_links_logic(
                client,
                message_context, # Original logic handles replies internally
                task_data["link"],
...
async def handle_torrent_link(client, message, link):
    ...
    try:
        ...
        # Add magnet and get GID
        gid = aria2.add_magnet(link, options={'pause': 'true'})
        
        # Monitor for metadata (Max 30 seconds)
        for _ in range(30):
            try:
                download = aria2.get_download(gid)
                if download.followed_by_ids:
                    # If metadata download created a new GID, switch to it
        ...
        if not download.files:
            # FIX: Use client.remove(gid) to avoid JSON serialization error
            try: aria2.client.remove(gid)
            except: pass
            return await msg.edit("❌ **Dead Torrent / No Metadata found.**\nTry downloading the .torrent file and sending that instead.")

        # Pause it while user selects files
        try: aria2.client.pause(gid)
        except: pass

        EXTERNAL_QUEUE[user_id] = {"gid": gid, "type": "torrent"}
        
        buttons = []
        # Show top 10 files (sorted by size)
        sorted_files = sorted(download.files, key=lambda x: x.length, reverse=True)[:10]
        for f in sorted_files:
            fname = Path(f.path).name
            if len(fname) > 30: fname = fname[:27] + "..."
            size = _pretty_bytes(f.length)
            # We use f.index (int) which is safe
...
# --- NEW: YouTube Quality Callback ---
@app.on_callback_query(filters.regex("^yt_q:"))
async def yt_quality_cb(client, query):
    user_id = query.from_user.id
    if user_id not in EXTERNAL_QUEUE: 
        return await query.answer("❌ Task expired. Send link again.", show_alert=True)
    ...
    await query.message.edit(f"✅ **Quality Selected:** `{quality}`\n⏳ **Queuing Task...**")
    
    # --- CLEANUP (Optional but recommended) ---
    del EXTERNAL_QUEUE[user_id] 
...
async def handle_general_link(client, message, link):
    ...
    if user_id in EXTERNAL_QUEUE:
        del EXTERNAL_QUEUE[user_id]

    # Options to just fetch metadata (fast)
    ...
    try:
        # Run extraction in a thread to not block bot
        def get_info():
        ...
        if 'entries' in info:
            ...
            EXTERNAL_QUEUE[user_id] = {"link": link, "type": "youtube", "is_playlist": True}
            
            # Simple quality selection for playlists
            buttons = [
                [InlineKeyboardButton("📺 1080p", callback_data="yt_q:1080"), InlineKeyboardButton("🖥️ 720p", callback_data="yt_q:720")],
                [InlineKeyboardButton("📼 480p", callback_data="yt_q:480"), InlineKeyboardButton("🎵 Audio", callback_data="yt_q:audio")],
                [InlineKeyboardButton("❌ Cancel", callback_data="close_menu")]
            ]
            await msg.edit(
                f"📋 **Playlist Found:** `{title}`\n"
                f"**Videos:** {count}\n\n"
                "Select quality for **ALL** videos:", 
                reply_markup=InlineKeyboardMarkup(buttons)
            )

        # B. SINGLE VIDEO LOGIC
        else:
            title = info.get('title', 'Video')
            formats = info.get('formats', [])
            available_qualities = set()
            
            # Scan for available resolutions
            for f in formats:
                if f.get('height'):
                    available_qualities.add(f['height'])
            
            # Sort qualities high to low
            sorted_q = sorted(list(available_qualities), reverse=True)
            ...
            for q in sorted_q:
                label = f"{q}p"
                if q >= 2160: label = "🌟 4K"
                elif q >= 1440: label = "✨ 2K"
                elif q == 1080: label = "📺 1080p"
                
                row.append(InlineKeyboardButton(label, callback_data=f"yt_q:{q}"))
                if len(row) == 2:
                    buttons.append(row)
                    row = []
            if row: buttons.append(row)
            
            buttons.append([InlineKeyboardButton("🎵 Audio Only (MP3)", callback_data="yt_q:audio")])
            buttons.append([InlineKeyboardButton("❌ Cancel", callback_data="close_menu")])

            EXTERNAL_QUEUE[user_id] = {"link": link, "type": "youtube", "is_playlist": False}
        ...
        print(f"YT Error: {e}")
        # Fallback to generic file mode if YT detection fails
        EXTERNAL_QUEUE[user_id] = {"link": link, "type": "file"}
        buttons = [[InlineKeyboardButton("📁 Upload as File", callback_data="ext_zip:no")]]
        await msg.edit("⚠️ **Could not fetch video info.**\nDownload as standard file?", reply_markup=InlineKeyboardMarkup(buttons))
...
class VirtualSplitFile:
    """
    Wrapper to upload 'chunks' of a large file without physically splitting it on disk.
    Saves Disk Space and RAM.
    """
    def __init__(self, path, start_byte, size_limit, part_name):
        self.path = path
    ...
    def read(self, chunk_size=64*1024):
        if self.current_offset >= self.limit:
            return b""
        to_read = min(chunk_size, self.limit - self.current_offset)
...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    # --- INTERNAL HELPER: UPLOAD LOGIC ---
    async def upload_files_in_folder(current_folder, file_list=None):
        if not file_list:
            file_list = [x for x in current_folder.rglob("*") if x.is_file()]
        ...
        for i, file_path in enumerate(file_list, start=1):
            ...
            asyncio.create_task(upstatus(client, status_message, message.chat.id, i, total_files))
            ...
            # Determine Method
            if ext in [".mp4", ".mkv", ".webm", ".avi"]: method = client.send_video
            elif ext in [".mp3", ".m4a", ".flac", ".wav", ".ogg"]: method = client.send_audio
            elif ext in [".jpg", ".png", ".jpeg"]: method = client.send_photo
            else: method = client.send_document
            ...
            while True:
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                try:
                    thumb_path = list(current_folder.glob("*.jpg"))
                    thumb = thumb_path[0] if thumb_path else None
                    
                    if fsize <= SPLIT_SIZE:
                        kwargs = {"chat_id": message.chat.id, "caption": caption, "progress": progress, "progress_args": [status_message, "up", task_uuid]}
                        if method == client.send_video:
                            kwargs["video"] = file_path
                            if thumb: kwargs["thumb"] = thumb
                            try:
                                import hachoir.parser
                                import hachoir.metadata
                                metadata = hachoir.metadata.extractMetadata(hachoir.parser.createParser(str(file_path)))
                                if metadata and metadata.has("duration"): kwargs["duration"] = int(metadata.get("duration").seconds)
                            except: pass
                        elif method == client.send_audio:
                             kwargs["audio"] = file_path
                             if thumb: kwargs["thumb"] = thumb
                        elif method == client.send_photo:
                        ...
                        start_byte = 0
                        part_num = 1
                        while start_byte < fsize:
                            chunk_size = min(SPLIT_SIZE, fsize - start_byte)
                            part_name = f"{file_path.name}.part{part_num:03d}"
                            with VirtualSplitFile(file_path, start_byte, chunk_size, part_name) as vf:
                                await client.send_document(
                                    message.chat.id, document=vf, file_name=part_name,
                                    caption=f"`{part_name}`", force_document=True,
                                    progress=progress, progress_args=[status_message, "up", task_uuid]
                                )
                    ...
                    upload_attempts += 1
                    if upload_attempts > 2: break
                    await asyncio.sleep(3)
            
            try: os.remove(file_path)
            except: pass
        for t in current_folder.glob("*.jpg"):
            try: os.remove(t)
            except: pass
    ...
    try:
        ...
        custom_name = task_data.get('custom_name', None)
        should_extract = task_data.get('extract', False)
        should_zip = task_data.get('zip', False)
        ...
        if ttype == "youtube":
            ...
            ydl_opts = {
                ...
                'allsubtitles': True,    # Download ALL languages
                'writethumbnail': True, 
                'addmetadata': True,
                'embedsubtitles': True,  # Embed inside MKV
            ...
            if quality == "audio":
                ...
                # Merges specific video quality with best audio into MKV
            ...
            await status_message.edit("🔎 **Fetching Info...**")
            entries = []
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(link, download=False)
                if 'entries' in info:
                    entries = list(info['entries'])
                    await status_message.edit(f"📋 **Playlist Detected:** {len(entries)} Videos\nStarting Loop: Download ➡️ Upload ➡️ Delete")
                else:
                    entries = [info]
            ...
            for idx, entry in enumerate(entries, 1):
                ...
                await status_message.edit(f"⬇️ **Downloading ({idx}/{total_items})**\n`{title}`")
                
                # Isolate download in a subfolder
                vid_folder = down_path / f"vid_{idx}"
                vid_folder.mkdir(exist_ok=True)
                ydl_opts['outtmpl'] = str(vid_folder / '%(title)s.%(ext)s')
                ...
                await status_message.edit(f"☁️ **Uploading ({idx}/{total_items})**\n`{title}`")
                await upload_files_in_folder(vid_folder)
                
                shutil.rmtree(vid_folder, ignore_errors=True)
                await asyncio.sleep(2)

        # ======================================================================
        # ☁️ MEGA / TORRENT / OTHER LOGIC
        # ======================================================================
        else:
            if ttype == 'torrent':
                gid = task_data['gid']
                ...
                while True:
                    ...
                    try:
                        ...
                        if (now - last_edit) > 6:
                            last_edit = now
                            perc = float(dl.progress) if dl.progress else 0.0
                            await status_message.edit(f"🧲 **Downloading Torrent...**\n`{generate_bar(perc)}` {perc:.2f}%")
                    except: break
                for f in dl.files:
                    if os.path.exists(f.path): shutil.move(f.path, str(down_path / Path(f.path).name))
                try: aria2.client.remove(gid)
                except: pass

            elif ttype == 'mega':
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                await status_message.edit("☁️ **Downloading from Mega...**")
                global m_login
                try: 
                    if not m_login: m_login = mega.login()
                except: pass
                if m_login: m_login.download_url(link, str(down_path))
                else: raise Exception("Mega Login Failed")

            else:
                # Direct / Generic Link
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                await status_message.edit("⬇️ **Direct Download...**")
                cmd = ["aria2c", link, "-d", str(down_path), "-x", "16", "-s", "16", "-k", "1M", "--check-certificate=false", "--user-agent=Mozilla/5.0"]
                if custom_name: cmd.extend(["-o", custom_name])
                
                proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
                await proc.wait()
                if proc.returncode != 0: raise Exception("Aria2 Failed")

            # --- PROCESSING (Extract/Zip) for Non-YouTube ---
            files = [x for x in down_path.rglob("*") if x.is_file()]
            if should_extract:
                await status_message.edit("📂 **Extracting...**")
                for f in files:
                    try:
                        proc = await asyncio.create_subprocess_shell(f"7z x '{f}' -o'{down_path}' -y")
            ...
            if should_zip:
                await status_message.edit("📦 **Zipping...**")
                zip_name = down_path / (custom_name if custom_name and custom_name.endswith(".zip") else f"{files[0].stem}.zip")
                with zipfile.ZipFile(zip_name, 'w', zipfile.ZIP_STORED) as zipf:
                    for f in files: zipf.write(f, arcname=f.name)
                for f in files: 
                     if str(f) != str(zip_name): os.remove(f)

            await status_message.edit("☁️ **Uploading...**")
            await upload_files_in_folder(down_path)

        await status_message.edit("✅ **Task Completed!**")

    except Exception as e:
        err = str(e)
        if "CANCELLED" in err:
            await status_message.edit("🛑 **Task Cancelled.**")
            if aria2_gid:
                try: aria2.remove([aria2_gid])
                except: pass
        else:
            await status_message.edit(f"❌ **Error:** `{err[:300]}`")
...
async def main():
    # 1. Start Health Check FIRST (Critical for Koyeb)
    # We start this immediately so Koyeb sees Port 8080 is open
    ...
    # 4. Connect to Telegram (This takes a few seconds, but Health Check is already running!)
...
# ==============================================================================
# --- BOT STARTUP (CRITICAL FIX) ---
# ==============================================================================

if __name__ == "__main__":
    print("🤖 Bot is starting...")
    # We must run the main() function, NOT app.run()
    # main() handles the Health Check + Bot connection logic
