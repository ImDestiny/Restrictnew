    ...
    if "https://t.me/" in text:
        ...
        try:
            ...
            for index, msgid in enumerate(range(fromID, toID+1), start=1):
                cycle_start = time.time() # Capture start time
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int, index: int, total_count: int, status_message: Message, dest_chat_id, dest_thread_id, delay, user_id, task_uuid=None):
    ...
    # ==================================================================
    # 1. FAST FORWARD (Copy Message)
    # ==================================================================
    
    if not msg.chat.has_protected_content:
        try:
            # Note: Bots can't copy files > 2GB usually.
            await acc.copy_message(chat_id=dest_chat_id, from_chat_id=chatid, message_id=msgid, message_thread_id=dest_thread_id)
            # FIX 1: Removed internal sleep(delay) here. 
            # The main loop handles the delay, preventing double-waiting.
            return True
        except FloodWait as e:
            # FIX 2: Do not swallow FloodWait. Raise it so the main loop waits properly.
            raise e
        except FileReferenceExpired:
            pass
        except Exception as e:
            print(f"Copy failed, falling back to download: {e}")
            pass

    if "Text" == msg_type:
        try: 
            await client.send_message(dest_chat_id, msg.text, entities=msg.entities, message_thread_id=dest_thread_id)
            # No sleep here either, main loop handles it.
        except: pass
        return True

    # ==================================================================
    # 2. DOWNLOAD & UPLOAD (Restricted Content)
    # ==================================================================
    ...
    try: 
        for attempt in range(3):
                            ...
                            for part in parts:
                                ...
                                while True:
                                    try:
                                        ...
                                        # Handle FloodWait in Split Upload
                                        await asyncio.sleep(e.value + 5)
                                    except Exception as e:
            ...
            # FIX 3: Catch FloodWait specifically during download
            except FloodWait as e:
                print(f"FloodWait during download: Sleeping {e.value}s")
                await asyncio.sleep(e.value + 5)
                # Loop continues to retry after sleep
                ...
                while True:
                    ...
                    try:
                        ...
                        if isinstance(e, FloodWait):
                            print(f"FloodWait during upload: Sleeping {e.value}s...")
                            await asyncio.sleep(e.value + 5)
                        else:
                            upload_attempts += 1
