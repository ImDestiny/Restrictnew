...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    try:
        ...
        custom_name = task_data.get('custom_name', None) # <--- GET NAME
        ...
        if ttype == 'torrent':
            ...
            while True:
                ...
                try:
                    ...
                    if dl.status == 'error': raise Exception("Torrent Error: Dead link/tracker.")
                    now = time.time()
                    if (now - last_edit) > 6:
                        last_edit = now
                        perc = float(dl.progress) if dl.progress else 0.0
                        msg = f"🧲 **Downloading Torrent...**\n`{generate_bar(perc)}` {perc:.2f}%"
                        await status_message.edit(msg)
                except Exception as e:
                    if "not found" in str(e): break
                    pass

            for f in dl.files:
                if os.path.exists(f.path):
                    target = down_path / Path(f.path).name
                    shutil.move(f.path, str(target))
            try: aria2.client.remove(gid)
            except: pass
            
            # TORRENT RENAME: If user gave -n and we have 1 file, rename it
            if custom_name:
                files_found = [x for x in down_path.rglob("*") if x.is_file()]
                if len(files_found) == 1:
                    new_path = down_path / custom_name
                    os.rename(files_found[0], new_path)
            ...
            if m_login:
                ...
                if custom_name:
                    files_found = [x for x in down_path.rglob("*") if x.is_file()]
                    if len(files_found) == 1:
                        os.rename(files_found[0], down_path / custom_name)
            else:
                raise Exception("Mega Login Failed")

        else:
            if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
            
            video_sites = ["youtube.com", "youtu.be", "instagram.com", "tiktok.com", "twitch.tv", "facebook.com", "vimeo.com", "dailymotion.com", "gofile.io", "mediafire.com"]
            is_video_site = any(site in link.lower() for site in video_sites)
            cookie_path = await db.get_cookies()

            if is_video_site:
                await status_message.edit("🔎 **Processing Link...**\n(Using yt-dlp)")
                qual = task_data.get('quality', 'best')
                
                # YT-DLP RENAME: Use outtmpl
                out_template = str(down_path / custom_name) if custom_name else str(down_path / '%(title)s.%(ext)s')
                ...
                elif qual == 'best': ydl_opts['format'] = 'bestvideo+bestaudio/best'
                else: ydl_opts['format'] = f"bestvideo[height<={qual}]+bestaudio/best/best"

                def run_video_download():
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ydl.download([link])
                await asyncio.to_thread(run_video_download)

            else:
                # DIRECT ARIA2 RENAME
                await status_message.edit(f"⬇️ **Direct Download...**" + (f"\nName: `{custom_name}`" if custom_name else ""))
                cmd = [
                    "aria2c", link,
                    "-d", str(down_path),
                    "-x", "16", "-s", "16", "-k", "1M",
                ...
                if custom_name:
                    ...
                    # Only use content-disposition if we are NOT renaming
                    cmd.append("--content-disposition-default-utf8=true")
                    
                if cookie_path:
                    cmd.extend(["--load-cookies", cookie_path])
                    ...
                    try:
                        ...
                        if "[" in line and "]" in line and "%" in line:
                            ...
                            if (now - last_edit) > 4:
                                ...
                                await status_message.edit(f"⬇️ **Aria2 Running...**\n`{stats}`")
                ...
                if process.returncode != 0:
                    ...
                    raise Exception(f"Aria2 Failed: {err.decode().strip()[:100]}")
        ...
        if not files: raise Exception("Download failed (No files found).")

        # 1. EXTRACTION (-e)
        if should_extract:
            await status_message.edit("📂 **Extracting Archive...**")
            for f in files:
                f_str = str(f)
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                try:
                    cmd = f"7z x '{f_str}' -o'{down_path}' -y"
                    proc = await asyncio.create_subprocess_shell(cmd)
                    await proc.communicate()
                    current_files = [x for x in down_path.rglob("*") if x.is_file()]
        ...
        # 2. ZIPPING (-z) - WITH RENAME SUPPORT
        if should_zip:
            await status_message.edit(f"📦 **Zipping Files...**")
            
            # ZIP RENAME: Use custom_name if provided, otherwise use first file name
            if custom_name:
                # Ensure .zip extension
                if not custom_name.endswith(".zip"): custom_name += ".zip"
                zip_name = down_path / custom_name
            else:
        ...
        for i, file_path in enumerate(files, start=1):
            ...
            while True:
                ...
                try:
                    ...
                    await status_message.edit(f"⏳ **FloodWait:** Sleeping {wait_time}s...")
