...
async def dl_handler(client: Client, message: Message):
    ...
    # --- PARSING ARGUMENTS ---
    # We split by space to find flags like -z, -e, -n
    cmd_parts = message.text.split()
    
    # Remove the command itself (/dl)
    if cmd_parts and cmd_parts[0].lower() == "/dl":
        cmd_parts.pop(0)
    ...
    # Iterate to find flags and clean the link
    clean_parts = []
    skip_next = False
    ...
    for i, part in enumerate(cmd_parts):
        ...
        if part.lower() == "-z":
            ...
            should_extract = True
        elif part.lower() in ["-n", "-n"]: # Checks for -n or -N
            if i + 1 < len(cmd_parts):
                custom_name = cmd_parts[i+1]
                skip_next = True # Skip the name in the next iteration
            else:
                return await message.reply("❌ **Error:** You must provide a name after `-n`.\nExample: `/dl link -n MyFile.mp4`")
        else:
            clean_parts.append(part)

    # --- FIND LINK ---
    # 1. Check if user Replied to a link
    if message.reply_to_message and (message.reply_to_message.text or message.reply_to_message.caption):
        link_text = message.reply_to_message.text or message.reply_to_message.caption
    # 2. Or check if link was left in the command
    elif len(clean_parts) > 0:
        link_text = clean_parts[0]
    ...
    if not link_text:
        return await message.reply_text(
            "❌ **Usage:**\n"
            "• `/dl link`\n"
            "• `/dl link -n NewName.mp4` (Rename)\n"
            "• `/dl link -z -n Archive.zip` (Zip & Rename)\n"
            "• Or Reply to a link with `/dl -n Name.ext`"
        )

    # Clean the link (remove surrounding text if any)
    ...
    if "https://t.me/" in link_text:
        ...
        if should_zip: status = f"🤐 **Zipping** as `{custom_name}`" if custom_name else "🤐 **Zipping...**"
        if custom_name and not should_zip: status = f"⬇️ **Downloading** as `{custom_name}`"
