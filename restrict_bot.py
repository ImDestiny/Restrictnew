...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    try:
        ...
        # --- B. MEGA.NZ ---
            ...
            global m_login
            if not m_login:
                try: m_login = mega.login()
                except: raise Exception("Mega Login Failed.")
            
            if task_data.get('mode') == 'all':
                m_login.download_url(link, str(down_path))
            else:
                m_login.download_url(link, str(down_path))

        # --- C. HYBRID ENGINE (SMART SWITCH) ---
        else:
            if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
            
            # Sites that REQUIRE yt-dlp to work (extraction logic)
            video_sites = ["youtube.com", "youtu.be", "instagram.com", "twitter.com", "x.com", "tiktok.com", "twitch.tv", "vimeo.com", "facebook.com", "pinterest.com", "gofile.io"]
            is_video_site = any(site in link.lower() for site in video_sites)
            cookie_path = await db.get_cookies()

            if is_video_site:
                # OPTION 1: USE YT-DLP (For Video Sites)
                await status_msg.edit("🎬 **Video Site Detected...**\nRunning yt-dlp...")
                ...
                ydl_opts = {
                    ...
                    'ignoreerrors': True,
                    'restrictfilenames': False # Fixes naming issues
                }
                ...
                # OPTION 2: PURE ARIA2 (For Direct Files / Generic Links)
                # This guarantees we get the real filename from headers!
                await status_msg.edit("⬇️ **Direct Link Detected...**\nRunning Pure Aria2...")
                cmd = [
                    "aria2c", link,
                    "-d", str(down_path),
                    "-x", "16", "-s", "16", "-k", "1M",
                    ...
                    "--content-disposition-default-utf8=true", # CRITICAL: Gets real name
                    ...
                    try:
                        ...
                        if "[" in line and "]" in line and "%" in line:
                            ...
                            if (now - last_edit) > 4:
                                ...
                                await status_msg.edit(f"⬇️ **Aria2 Running...**\n`{stats}`")
        ...
        if should_extract:
            ...
            for f in files:
                f_str = str(f)
                # FIX: Removed extension check. Extracts ANYTHING if -e is used.
                if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                try:
                    cmd = f"7z x '{f_str}' -o'{down_path}' -y"
                    proc = await asyncio.create_subprocess_shell(cmd)
                    await proc.communicate()
                    # Delete original archive only if we extracted something
        ...
        # 2. ZIPPING (-z)
        if should_zip:
            await status_msg.edit("📦 **Zipping Files...**")
            # Get the name of the first file to name the zip
            zip_name = down_path / f"{files[0].stem}.zip"
            if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
            
            # FIX: Use Python's built-in zipfile (No external dependency needed)
            with zipfile.ZipFile(zip_name, 'w', zipfile.ZIP_STORED) as zipf:
                for f in files:
                    if f != zip_name:
                        # arcname writes the file with just its filename, not full path
        ...
        for i, file_path in enumerate(files, start=1):
            ...
            if fsize <= SPLIT_SIZE:
                await client.send_document(
                    ...
                    force_document=True,
                    progress=progress, 
                    progress_args=[status_msg, "up", task_uuid]
                )
            else:
                start_byte = 0
                part_num = 1
        ...
        if "CANCELLED" in err:
            ...
            await status_msg.edit(f"❌ **Error:** `{err[:300]}`")
