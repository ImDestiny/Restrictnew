...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    try:
                ...
                try:
                    ...
                    if (now - last_edit) > 6:
                        ...
                        msg = (
                            ...
                            f"**{perc:.2f}%** │ `{generate_bar(perc)}`\n\n"
                            f"🚀 **Speed:** `{_pretty_bytes(dl.download_speed)}/s`\n"
                            f"💾 **Size:** `{_pretty_bytes(dl.completed_length)} / {_pretty_bytes(dl.total_length)}`\n"
                            f"⏳ **ETA:** `{str(dl.eta)}`\n"
                            f"📡 **Seeds:** `{dl.num_seeders}`"
                        )
                        await status_msg.edit(msg)
                except Exception as e:
                    if "not found" in str(e): break # Download finished/removed
                    pass
        ...
        # --- B. MEGA.NZ ---
            ...
            try:
                ...
                if not m_login:
                    ...
                    except: raise Exception("Mega Login Failed. Check Config.")
                ...
                raise Exception(f"Mega API Error: {e}")

        # --- C. HTTP / YOUTUBE / DRIVE (Yt-dlp + Aria2) ---
        else:
            if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
            ...
            # Hook to capture yt-dlp progress
            def progress_hook(d):
                if d['status'] == 'downloading':
                    try:
                        ...
                        total = d.get('_total_bytes_str') or d.get('_total_bytes_estimate_str') or "Unknown"
                        
                        # Write to file or variable to update TG (Optional optimization)
                        # For now, we rely on Aria2 console output or just show generic status
                        # because updating TG message from inside a thread is unsafe/rate-limited.
                        pass 
                    except: pass
            ...
            ydl_opts = {
                ...
                'external_downloader_args': ['-x', '8', '-k', '1M', '-s', '8'], # Balanced config
                'socket_timeout': 60,
                'ignoreerrors': True,
            ...
            # Format selection
            # FIX: Handle 'best' quality explicitly to avoid height<=best error
            if qual == 'audio': 
                ydl_opts['format'] = 'bestaudio/best'
            elif qual == 'best': 
                ydl_opts['format'] = 'bestvideo+bestaudio/best' # Just get the best
            elif qual != 'direct': 
                ydl_opts['format'] = f"bestvideo[height<={qual}]+bestaudio/best/best"                

            await status_msg.edit("⬇️ **Downloading...**\n(Using Aria2 Engine)")
            
            def run_download():
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    # 1. Try to get filename first
                    try:
                        info = ydl.extract_info(link, download=False)
                    except: 
                        # Fallback for direct non-meta links
                        fname = link.split("/")[-1].split("?")[0]
                        if not fname: fname = "file.dat"
                        ydl_opts['outtmpl'] = str(down_path / fname)
        ...
        if should_extract:
            ...
            for f in files:
                f_str = str(f)
                if f_str.endswith((".zip", ".rar", ".7z", ".tar", ".gz")):
                    if CANCEL_FLAGS.get(task_uuid): raise Exception("CANCELLED_BY_USER")
                    try:
                        cmd = f"7z x '{f_str}' -o'{down_path}' -y"
                        proc = await asyncio.create_subprocess_shell(cmd)
                        await proc.communicate()
                        os.remove(f) # Delete archive
                    except Exception as e:
                        print(f"Extraction Error: {e}")
            # Refresh list
            files = [x for x in down_path.rglob("*") if x.is_file()]

        # 2. ZIPPING (User requested Zip)
        if should_zip:
        ...
        # ======================================================================
        # ☁️ UPLOAD PHASE (Virtual Split)
        # ======================================================================
        
        total_files = len(files)
        # Start Upload Progress Tracker
        asyncio.create_task(upstatus(client, status_msg, message.chat.id, 0, total_files))
        
        # Determine Split Size based on Premium
        SPLIT_SIZE = 2000 * 1024 * 1024
        try:
            # Check if user session (acc) is available and premium
            # (Requires logic to check 'acc' variable, usually passed or global)
            # Default to 2GB for bot safety
            pass 
        except: pass
        ...
        for i, file_path in enumerate(files, start=1):
            ...
            if fsize <= SPLIT_SIZE:
                ...
                # --- VIRTUAL SPLIT UPLOAD (No RAM/Disk Load) ---
                start_byte = 0
                part_num = 1
                ...
                while start_byte < fsize:
                    ...
                    part_caption = f"`{part_name}`"
                    
                    # Stream directly from Disk -> Network
            ...
            # Delete file after upload to save space immediately
        ...
        # Cleanup Active Process Tracker
        if task_uuid in ACTIVE_PROCESSES.get(user_id, {}):
             del ACTIVE_PROCESSES[user_id][task_uuid]
        batch_temp.ACTIVE_TASKS[user_id] -= 1
