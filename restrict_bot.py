...
m_login = None
try:
    # FIX: Only try to login if variables are actually set and not empty
    if MEGA_EMAIL and len(str(MEGA_EMAIL).strip()) > 0 and MEGA_PASSWORD and len(str(MEGA_PASSWORD).strip()) > 0:
        print("🔐 Logging into Mega Account...")
        m_login = mega.login(MEGA_EMAIL, MEGA_PASSWORD)
    else:
        print("👤 Logging into Mega Anonymously...")
        m_login = mega.login()
