...
async def dl_handler(client: Client, message: Message):
    ...
    if "https://t.me/" in link_text:
        ...
        # FIX: Send the message HERE, then pass IT to start_task_final
        # This prevents the bot from trying to reply to the user message later
        status_msg = await message.reply(f"⬇️ **Queued External Download**\nExtract: `{should_extract}`")
    ...
    try:
        if isinstance(message_context, Message):
            if message_context.from_user.is_bot:
                # It's already a bot message (e.g. from callback), edit it
                await message_context.edit(start_text)
                status_message = message_context
            else:
                # It's a user message (e.g. /dl), REPLY to it and capture the new msg
                status_message = await message_context.reply(start_text, quote=True)
    except Exception as e:
        print(f"Error sending start message: {e}")
        # Fallback if we can't send/edit
        status_message = message_context 
...
async def process_external_logic(client, message, task_data, delay, user_id, task_uuid):
    ...
    try:
        ...
        # --- C. HTTP / YOUTUBE / DRIVE (Yt-dlp + Aria2) ---
            ...
            elif qual != 'direct': ydl_opts['format'] = f"bestvideo[height<={qual}]+bestaudio/best/best"
